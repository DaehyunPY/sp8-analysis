...
MyHistos::MyHistos(const bool verbose, int size)
    : fVerbose(verbose), rootfile(0) {
  arsize = size;
...
TH1 *MyHistos::create3d(int id,
  ...
  if (obj)
    std::cout << "name doesn't match(" << id << ") is:" << obj->GetName()
        << " should be:" << hist3->GetName() << std::endl;

  //--now add it to the list--//
  hiarray[id] = hist3;
  if (fVerbose)
    std::cout << "create 3D: " << dir << "/" << hist3->GetName() << std::endl;
  return hist3;
}

void MyHistos::fill(int id,
                    const char *name,
                    double fillX,
                    double fillY,
                    double fillZ,
  ...
  if (!hist3) {
  ...
  else if ((!hist3->GetEntries()) && ((nXbins != hist3->GetXaxis()->GetNbins())
      || (xLow != hist3->GetXaxis()->GetXmin())
      || (xUp != hist3->GetXaxis()->GetXmax()) ||
      (nYbins != hist3->GetYaxis()->GetNbins())
      || (yLow != hist3->GetYaxis()->GetXmin())
      || (yUp != hist3->GetYaxis()->GetXmax()) ||
      (nZbins != hist3->GetZaxis()->GetNbins())
    ...
    std::cout << "name doesn't match(" << id << ") is:" << hist3->GetName()
...
TH1 *MyHistos::create2d(int id, const char *name,
  ...
  if (obj)
    std::cout << "name doesn't match(" << id << ") is:" << obj->GetName()
        << " should be:" << hist2->GetName() << std::endl;

  //--now add it to the list--//
  hiarray[id] = hist2;
  if (fVerbose)
    std::cout << "create 2D: " << dir << "/" << hist2->GetName() << std::endl;
  return hist2;
}
void MyHistos::fill(int id, const char *name, double fillX, double fillY,
  ...
  if (!hist2) {
    ...
    //if the histogram exists, but has no entries, we want to resize and rebin it//
  else if ((!hist2->GetEntries()) && ((nXbins != hist2->GetXaxis()->GetNbins())
      || (xLow != hist2->GetXaxis()->GetXmin())
      || (xUp != hist2->GetXaxis()->GetXmax()) ||
      (nYbins != hist2->GetYaxis()->GetNbins())
      || (yLow != hist2->GetYaxis()->GetXmin())
      || (yUp != hist2->GetYaxis()->GetXmax())))
    hist2->SetBins(nXbins, xLow, xUp, nYbins, yLow, yUp);

    //--if the histogram exists, check also if given name is the one of the histogram--//
  else if (strcmp(hist2->GetName(), name))
    std::cout << "name doesn't match(" << id << ") is:" << hist2->GetName()
        << " should be:" << name << std::endl;
...
TH1 *MyHistos::create1d(int id, const char *name,
  ...
  if (obj)
    std::cout << "name doesn't match(" << id << ") is:" << obj->GetName()
        << " should be:" << hist1->GetName() << std::endl;

  //--now add it to the list--//
  hiarray[id] = hist1;
  if (fVerbose)
    std::cout << "create 1D: " << dir << "/" << hist1->GetName() << std::endl;
  return hist1;
}
void MyHistos::fill(int id, const char *name, double fillX, const char *titleX,
  ...
  if (!hist1) {
    ...
    //if the histogram exists, but has no entries, we want to resize and rebin it//
  else if ((!hist1->GetEntries()) && ((nXbins != hist1->GetXaxis()->GetNbins())
      || (xLow != hist1->GetXaxis()->GetXmin())
      || (xUp != hist1->GetXaxis()->GetXmax())))
    hist1->SetBins(nXbins, xLow, xUp);

    //--if the histogram exists, check also if given name is the one of the histogram--//
  else if (strcmp(hist1->GetName(), name))
    std::cout << "name doesn't match(" << id << ") is:" << hist1->GetName()
        << " should be:" << name << std::endl;
...
void MyHistos::LinkRootFile(TFile &RootFile) {
  rootfile = &RootFile;
}
