...
bool read_config_file(char * name, sort_class *& sorter, int& command, double& offset_sum_u, double& offset_sum_v, double& offset_sum_w, double& w_offset, double& pos_offset_x, double& pos_offset_y)
{
...
//////////////////////////////////////////////////////////////////////////////////////////
{
	...
	int root_argc = 2;
	sprintf(argv2,"-l");
	root_argv[0] = argv[0];	root_argv[1] = argv2;	root_argv[2] = argv3;
	...
	if (!read_config_file("sorter_ion.txt", ion_sorter, ion_command, ion_offset_sum_u, ion_offset_sum_v, ion_offset_sum_w, ion_w_offset, ion_pos_offset_x, ion_pos_offset_y)) {
		delete ion_sorter; ion_sorter = 0;
	}
	if (ion_sorter) {
	...
	if (!read_config_file("sorter_elec.txt", elec_sorter, elec_command, elec_offset_sum_u, elec_offset_sum_v, elec_offset_sum_w, elec_w_offset, elec_pos_offset_x, elec_pos_offset_y)) {
		delete elec_sorter; elec_sorter = 0;
	}
	if (elec_sorter) {
	...
	if (ion_sorter) {
		...
		if (ion_command >= 2) {
			ion_sorter->create_scalefactors_calibrator(true, ion_sorter->runtime_u, ion_sorter->runtime_v, ion_sorter->runtime_w, 0.78, ion_sorter->fu, ion_sorter->fv, ion_sorter->fw); 
		}
	...
	if (elec_sorter) {
		...
		if (elec_command >= 2) {
			elec_sorter->create_scalefactors_calibrator(true, elec_sorter->runtime_u, elec_sorter->runtime_v, elec_sorter->runtime_w, 0.78, elec_sorter->fu, elec_sorter->fv, elec_sorter->fw); 
		}
	...
	if (ion_sorter && fill_histograms) {
		...
		ion_canvas->cd(1); Hion_sum_u = rt->newTH1D("ion_sum_u","",500*10,-250,250.); Hion_sum_u->Draw();
		ion_canvas->cd(2); Hion_sum_v = rt->newTH1D("ion_sum_v","",500*10,-250,250.); Hion_sum_v->Draw();
		ion_canvas->cd(3); Hion_sum_w = rt->newTH1D("ion_sum_w","",500*10,-250,250.); Hion_sum_w->Draw();
		ion_canvas->cd(4); Hion_u = rt->newTH1D("ion_u","",500*2,-250,250.); Hion_u->Draw();
		ion_canvas->cd(5); Hion_v = rt->newTH1D("ion_v","",500*2,-250,250.); Hion_v->Draw();
		ion_canvas->cd(6); Hion_w = rt->newTH1D("ion_w","",500*2,-250,250.); Hion_w->Draw();
		ion_canvas->cd(7); Hion_xy_raw = rt->newTH2D("ion_xy_raw","",120*4,-60,60.,120*4,-60,60.,"COLZ"); Hion_xy_raw->Draw();
		ion_canvas->cd(8); Hion_xy = rt->newTH2D("ion_xy","",120*4,-60,60.,120*4,-60,60.,"COLZ"); Hion_xy->Draw();
		int s = 100;
		ion_canvas->cd(9); Hion_xy_dev = rt->newTH2D("ion_xy_dev","",s*2,-double(s),double(s),2*s,-double(s),double(s),"COLZ"); Hion_xy_dev->Draw();
	}

	if (elec_sorter && fill_histograms) {
		elec_canvas = rt->newCanvas("elec_canvas","elec_canvas",10,10,910,910);
		elec_canvas->Divide(3,3);
		elec_canvas->cd(1); Helec_sum_u = rt->newTH1D("elec_sum_u","",500*10,-250,250.); Helec_sum_u->Draw();
		elec_canvas->cd(2); Helec_sum_v = rt->newTH1D("elec_sum_v","",500*10,-250,250.); Helec_sum_v->Draw();
		elec_canvas->cd(3); Helec_sum_w = rt->newTH1D("elec_sum_w","",500*10,-250,250.); Helec_sum_w->Draw();
		elec_canvas->cd(4); Helec_u = rt->newTH1D("elec_u","",500*2,-250,250.); Helec_u->Draw();
		elec_canvas->cd(5); Helec_v = rt->newTH1D("elec_v","",500*2,-250,250.); Helec_v->Draw();
		elec_canvas->cd(6); Helec_w = rt->newTH1D("elec_w","",500*2,-250,250.); Helec_w->Draw();
		elec_canvas->cd(7); Helec_xy_raw = rt->newTH2D("elec_xy_raw","",120*4,-60,60.,120*4,-60,60.,"COLZ"); Helec_xy_raw->Draw();
		elec_canvas->cd(8); Helec_xy = rt->newTH2D("elec_xy","",120*4,-60,60.,120*4,-60,60.,"COLZ"); Helec_xy->Draw();
		int s = 100;
		elec_canvas->cd(9); Helec_xy_dev = rt->newTH2D("elec_xy_dev","",s*2,-double(s),double(s),2*s,-double(s),double(s),"COLZ"); Helec_xy_dev->Draw();
	}
	...
	if (ion_command == 0 || ion_command == 1 || elec_command == 0 || elec_command == 1) {
		...
		if (!outfile) {printf("could not open output file.\n"); outfile = 0; return false;}
		if (ferror(outfile)) {printf("could not open output file.\n"); outfile = 0; return false;}
	}
	...
	while (true) {
		if (LMF->GetEventNumber()%20000 == 1) {
			if (my_kbhit()) break;
			gSystem->ProcessEvents(); // allow the system to show the histograms
			printf("\rreading event data... %2i %c  ",__int32(100*LMF->GetEventNumber()/LMF->uint64_Numberofevents),37);
		...
		if (ion_sorter) {
			if (ion_sorter->Cmcp > -1) {
				for (unsigned int i = 0; i<count[ion_sorter->Cmcp];++i) tdc_ns[ion_sorter->Cmcp][i] = double(TDC[ion_sorter->Cmcp][i])  * tdcresolution;
			}
			for (unsigned int i = 0; i<count[ion_sorter->Cu1];++i) tdc_ns[ion_sorter->Cu1][i]  = double(TDC[ion_sorter->Cu1][i] )  * tdcresolution;
			for (unsigned int i = 0; i<count[ion_sorter->Cu2];++i) tdc_ns[ion_sorter->Cu2][i]  = double(TDC[ion_sorter->Cu2][i] )  * tdcresolution;
			for (unsigned int i = 0; i<count[ion_sorter->Cv1];++i) tdc_ns[ion_sorter->Cv1][i]  = double(TDC[ion_sorter->Cv1][i] )  * tdcresolution;
			for (unsigned int i = 0; i<count[ion_sorter->Cv2];++i) tdc_ns[ion_sorter->Cv2][i]  = double(TDC[ion_sorter->Cv2][i] )  * tdcresolution;
			if (ion_sorter->use_HEX) {
				for (unsigned int i = 0; i<count[ion_sorter->Cw1];++i) tdc_ns[ion_sorter->Cw1][i]  = double(TDC[ion_sorter->Cw1][i] )  * tdcresolution;
				for (unsigned int i = 0; i<count[ion_sorter->Cw2];++i) tdc_ns[ion_sorter->Cw2][i]  = double(TDC[ion_sorter->Cw2][i] )  * tdcresolution;
			}
			if (ion_sorter->use_HEX) {
			...
			ion_sorter->feed_calibration_data(true, ion_w_offset); // for calibration of fv, fw, w_offset and correction tables
			if (ion_sorter->scalefactors_calibrator && ion_command >= 2) {
				if (ion_sorter->scalefactors_calibrator->map_is_full_enough()) break;
			}
			if (ion_sorter->use_HEX && fill_histograms) Hion_xy_dev->Fill(ion_sorter->scalefactors_calibrator->binx-ion_sorter->scalefactors_calibrator->detector_map_size/2, ion_sorter->scalefactors_calibrator->biny-ion_sorter->scalefactors_calibrator->detector_map_size/2, ion_sorter->scalefactors_calibrator->detector_map_devi_fill);
			...
			if (count[ion_sorter->Cu1] > 0 && count[ion_sorter->Cu2] > 0) {
				...
				if (fill_histograms) Hion_sum_u->Fill(tdc_ns[ion_sorter->Cu1][0] + tdc_ns[ion_sorter->Cu2][0] - 2*mcp);
			}
			if (count[ion_sorter->Cv1] > 0 && count[ion_sorter->Cv2] > 0) {
				...
				if (fill_histograms) Hion_sum_v->Fill(tdc_ns[ion_sorter->Cv1][0] + tdc_ns[ion_sorter->Cv2][0] - 2*mcp);
			}
			if (ion_sorter->use_HEX) {
				if (count[ion_sorter->Cw1] > 0 && count[ion_sorter->Cw2] > 0) {
					...
					if (fill_histograms) Hion_sum_w->Fill(tdc_ns[ion_sorter->Cw1][0] + tdc_ns[ion_sorter->Cw2][0] - 2*mcp);
				}
			}
		}
		...
		if (elec_sorter) {
			if (elec_sorter->Cmcp > -1) {
				for (unsigned int i = 0; i<count[elec_sorter->Cmcp];++i) tdc_ns[elec_sorter->Cmcp][i] = double(TDC[elec_sorter->Cmcp][i])  * tdcresolution;
			}
			for (unsigned int i = 0; i<count[elec_sorter->Cu1];++i) tdc_ns[elec_sorter->Cu1][i]  = double(TDC[elec_sorter->Cu1][i] )  * tdcresolution;
			for (unsigned int i = 0; i<count[elec_sorter->Cu2];++i) tdc_ns[elec_sorter->Cu2][i]  = double(TDC[elec_sorter->Cu2][i] )  * tdcresolution;
			for (unsigned int i = 0; i<count[elec_sorter->Cv1];++i) tdc_ns[elec_sorter->Cv1][i]  = double(TDC[elec_sorter->Cv1][i] )  * tdcresolution;
			for (unsigned int i = 0; i<count[elec_sorter->Cv2];++i) tdc_ns[elec_sorter->Cv2][i]  = double(TDC[elec_sorter->Cv2][i] )  * tdcresolution;
			if (elec_sorter->use_HEX) {
				for (unsigned int i = 0; i<count[elec_sorter->Cw1];++i) tdc_ns[elec_sorter->Cw1][i]  = double(TDC[elec_sorter->Cw1][i] )  * tdcresolution;
				for (unsigned int i = 0; i<count[elec_sorter->Cw2];++i) tdc_ns[elec_sorter->Cw2][i]  = double(TDC[elec_sorter->Cw2][i] )  * tdcresolution;
			}
			if (elec_sorter->use_HEX) {
				// shift the time sums to zero:
			...
			elec_sorter->feed_calibration_data(true, elec_w_offset); // for calibration of fv, fw, w_offset and correction tables
			if (elec_sorter->scalefactors_calibrator && elec_command >= 2) {
				if (elec_sorter->scalefactors_calibrator->map_is_full_enough()) break;
			}
			if (elec_sorter->use_HEX && fill_histograms) Helec_xy_dev->Fill(elec_sorter->scalefactors_calibrator->binx-elec_sorter->scalefactors_calibrator->detector_map_size/2, elec_sorter->scalefactors_calibrator->biny-elec_sorter->scalefactors_calibrator->detector_map_size/2, elec_sorter->scalefactors_calibrator->detector_map_devi_fill);
		...
		if (ion_sorter) {	
			...
			for (int i=0;i<number_of_ions;i++) {
				if (fill_histograms) Hion_xy->Fill(ion_sorter->output_hit_array[i]->x, ion_sorter->output_hit_array[i]->y);
			}
		}
		...
		if (elec_sorter) {
			...
			for (int i=0;i<number_of_electrons;i++) {
				if (fill_histograms) Helec_xy->Fill(elec_sorter->output_hit_array[i]->x, elec_sorter->output_hit_array[i]->y);
			}
		}
		...
		if (outfile) { // write to output file:
			if (ion_sorter) {
				...
				if ( ion_sorter->use_HEX) ion_sorter->shift_sums(-1, ion_offset_sum_u, ion_offset_sum_v, ion_offset_sum_w);
				...
				if (ion_sorter->Cmcp > -1) {
					for (unsigned int i = 0; i<count[ion_sorter->Cmcp];++i) TDC[ion_sorter->Cmcp][i] = int(tdc_ns[ion_sorter->Cmcp][i] / tdcresolution);
				}
				for (unsigned int i = 0; i<count[ion_sorter->Cu1];++i) TDC[ion_sorter->Cu1][i] = int(tdc_ns[ion_sorter->Cu1][i] / tdcresolution);
				for (unsigned int i = 0; i<count[ion_sorter->Cu2];++i) TDC[ion_sorter->Cu2][i] = int(tdc_ns[ion_sorter->Cu2][i] / tdcresolution);
				for (unsigned int i = 0; i<count[ion_sorter->Cv1];++i) TDC[ion_sorter->Cv1][i] = int(tdc_ns[ion_sorter->Cv1][i] / tdcresolution);
				for (unsigned int i = 0; i<count[ion_sorter->Cv2];++i) TDC[ion_sorter->Cv2][i] = int(tdc_ns[ion_sorter->Cv2][i] / tdcresolution);
				if (ion_sorter->use_HEX) {
					for (unsigned int i = 0; i<count[ion_sorter->Cw1];++i) TDC[ion_sorter->Cw1][i] = int(tdc_ns[ion_sorter->Cw1][i] / tdcresolution);
					for (unsigned int i = 0; i<count[ion_sorter->Cw2];++i) TDC[ion_sorter->Cw2][i] = int(tdc_ns[ion_sorter->Cw2][i] / tdcresolution);
				}
			}

			if (elec_sorter) {
				// shift the detector signals back (note the -1 instead of the +1)
				if ( elec_sorter->use_HEX) elec_sorter->shift_sums(-1, elec_offset_sum_u, elec_offset_sum_v, elec_offset_sum_w);
				...
				// convert the times from nanoseconds back to raw channels:
				if (elec_sorter->Cmcp > -1) {
					for (unsigned int i = 0; i<count[elec_sorter->Cmcp];++i) TDC[elec_sorter->Cmcp][i] = int(tdc_ns[elec_sorter->Cmcp][i] / tdcresolution);
				}
				for (unsigned int i = 0; i<count[elec_sorter->Cu1];++i) TDC[elec_sorter->Cu1][i] = int(tdc_ns[elec_sorter->Cu1][i] / tdcresolution);
				for (unsigned int i = 0; i<count[elec_sorter->Cu2];++i) TDC[elec_sorter->Cu2][i] = int(tdc_ns[elec_sorter->Cu2][i] / tdcresolution);
				for (unsigned int i = 0; i<count[elec_sorter->Cv1];++i) TDC[elec_sorter->Cv1][i] = int(tdc_ns[elec_sorter->Cv1][i] / tdcresolution);
				for (unsigned int i = 0; i<count[elec_sorter->Cv2];++i) TDC[elec_sorter->Cv2][i] = int(tdc_ns[elec_sorter->Cv2][i] / tdcresolution);
				if (elec_sorter->use_HEX) {
					for (unsigned int i = 0; i<count[elec_sorter->Cw1];++i) TDC[elec_sorter->Cw1][i] = int(tdc_ns[elec_sorter->Cw1][i] / tdcresolution);
					for (unsigned int i = 0; i<count[elec_sorter->Cw2];++i) TDC[elec_sorter->Cw2][i] = int(tdc_ns[elec_sorter->Cw2][i] / tdcresolution);
				}
			}
	...
	if (ion_command == 2) {
		...
		if (ion_sorter->scalefactors_calibrator) {
			printf("ion: Good scalefactors are:\nf_U = %lg\nf_V = %lg\nf_W = %lg\nOffset on layer W = %lg\n", 2.*ion_sorter->fu, 2.*ion_sorter->scalefactors_calibrator->best_fv, 2.*ion_sorter->scalefactors_calibrator->best_fw, ion_sorter->scalefactors_calibrator->best_w_offset);
		}
	}
	...
	if (elec_command == 2) {
		...
		if (elec_sorter->scalefactors_calibrator) {
			printf("elec: Good scalefactors are:\nf_U = %lg\nf_V = %lg\nf_W = %lg\nOffset on layer W = %lg\n", 2.*elec_sorter->fu, 2.*elec_sorter->scalefactors_calibrator->best_fv, 2.*elec_sorter->scalefactors_calibrator->best_fw, elec_sorter->scalefactors_calibrator->best_w_offset);
		}
	}
