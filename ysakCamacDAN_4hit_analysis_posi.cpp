...
#include <math.h>
#include <fstream>
FILE *fp1, *fp_cor, *fp_sel, *fp_mom, *fp_tes ;
std::ofstream testf;
...
double elcPsiLim, elecnnPzLimit;
...
CDAN_API BOOL AnalysisInitialize(CDoubleArray *pEventData,CDoubleArray *pParameters, CDoubleArray *pWeighParameter)
{
  ...
  elcPsiLim           = pParameters->GetAt(247-1)*pi/180.0;
  elecnnPzLimit  = pParameters->GetAt(248-1);
...
//		fprintf(fp1, "%lf \n", dPosi_cor[i]);
//	}
    fp_sel = fopen("SelLuc.dat","ab");

	fp_mom = fopen("Momenta.dat","ab");
	
	fp_tes = fopen("TEST.dat","at");
	testf.open("TEST3.dat",std::ios::trunc);
...
{
	iTest++;
	if (iTest<=10000) //(!((iTest == 6)||(iTest ==24)||(iTest == 37)||(iTest == 153)||(iTest == 153)||(iTest == 170)))     return;
	testf << "Event:" <<iTest << std::endl;
	// for Hexagonal anode
...
/////////////////////////////////////////////////////////////////////////////
	...
	if((nMCPele==0) || (nMCPion==0))  //If the number of electrons and ions is not within requirement
	{
		...
		testf << "MCPにイオンと電子来てます　675突破" << std::endl;//C++の簡単な表現方法。上と同じ
...
/////////////////////////////////////////////////////////////////
	...
	// nHit+1->nHit
	for(iHit=0;iHit<nHit;iHit++){
		b[0][iHit] = TDC[1][1][iHit];		// x1 (ns)
		b[1][iHit] = TDC[2][0][iHit];		// x2 (ns)
...
// Now do calculates for the ion's position of all hits
	...
	testf << "dx0:" <<dx0 << std::endl;
	testf << "dy0:" <<dy0 << std::endl;
	testf << "dySq[0]:" <<dySq[0] << std::endl;
	testf << "dxSq[0]:" <<dxSq[0] << std::endl;
	testf << "dySq[1]:" <<dySq[1] << std::endl;
	testf << "dxSq[1]:" <<dxSq[1] << std::endl;	
	
	testf << "iAnaIon:" <<iAnaIon << std::endl;
	testf << "iHitsTDC2:" <<iHitsTDC2 << std::endl;
...
//--------------------------------------------------------------------------
	...
	testf << "730突破" << std::endl;//C++の簡単な表現方法。上と同じ
	for(iHit=0;iHit<iAnaIon;iHit++)  //
	{
	...
	testf << dtIon[iHit] << std::endl;//C++の簡単な表現方法。上と同じ //09/08 Ne cluster用に追加
	if(iMaster_Flag<0) goto hell; // select the ion from Time of Flight. 
	testf << "747突破" << std::endl;//C++の簡単な表現方法。上と同じ
	isellect=0;
...
//		dCOM_z[jjj] = tc0/2.0/dMass[0]*(dCharge[0]*dElectric_Field*tc0+an*sqrt(8.0)*sqrtA)-dL;
      ...
      dSum_Of_myt = dSum_Of_myt+dySqN[jjj][iHit]*dMass[iHit]/dtIonN[jjj][iHit]; 
		...
		for(iHit=0;iHit<iAnaIon;iHit++)
		{
			...
			// 13/09/13 ３体問題の取り扱いのため２イオンで求めた反応中心を使用しない  **ここから**
			...
			// 13/09/13 ３体問題の取り扱いのため２イオンで求めた反応中心を使用しない  **ここまで**
		}
	}  //end of jjj
...
ion1:
	testf << "fabs(dCOM_z[0])" << fabs(dCOM_z[0]) << std::endl;;//C++の簡単な表現方法。上と同じ
	testf << "fabs(dCOM_z[1])" << fabs(dCOM_z[1]) << std::endl;;//C++の簡単な表現方法。上と同じ
	testf << "iMaster_Flag_beforeion2" << iMaster_Flag << std::endl;;//C++の簡単な表現方法。上と同じ
	testf << "flagmatrix_beforeion2" << flagmatrix << std::endl;;//C++の簡単な表現方法。上と同じ
	jjj=1;
	if(fabs(dCOM_x[jjj] - dCOM_x0) > dCOM_x_width){
		flagmatrix = 11;
...
ion2:
	testf << "iMaster_Flag" << iMaster_Flag << std::endl;;//C++の簡単な表現方法。上と同じ
	testf << "flagmatrix" << flagmatrix << std::endl;;//C++の簡単な表現方法。上と同じ
	if(iMaster_Flag < -12 )
	{
		goto hell;
	}
	...
	if(iMaster_Flag > -2 )
	{
		...
		isellect=3;
		for(jjj=0; jjj<2; jjj++){
 			//dSum_of_momenta[jjj] = dSum_of_momenta_Z[jjj]*dSum_of_momenta_Z[jjj] + dSum_of_momenta_Y[jjj]*dSum_of_momenta_Y[jjj] + dSum_of_momenta_X[jjj]*dSum_of_momenta_X[jjj];
	...
	iMaster_Flag = 0;
	testf << "928突破" << std::endl;//C++の簡単な表現方法。上と同じ
...
// end of check ion 
	...
	testf << "950突破" << std::endl;//C++の簡単な表現方法。上と同じ
	...
	if(iMaster_Flag < 0) goto hell;
	testf << "976突破" << std::endl;//C++の簡単な表現方法。上と同じ
...
// selection of electron TOF, the first electron
	...
	testf << "1073突破" << std::endl;//C++の簡単な表現方法。上と同じ
...
//   calculation of Pz
		...
		//find_elec_pz(dDelta_T, -dCOM_z[jn], dElectronPz[iCount]); //Pzの補正　dIonPzと同様　13/09/19山田
		...
		testf << "1093突破" << std::endl;//C++の簡単な表現方法。上と同じ
// calculation of Px, Py
		...
		testf << "1165突破" << std::endl;//C++の簡単な表現方法。上と同じ
...
//			theta = double(it)*dAngle_Width/2;
  ...
  else if(iPolarization==1)
  {
	...
	if((dIon_Total_Energy > ionEngMin) && (dIon_Total_Energy < ionEngMax) && 
	    ...
	    //&& (((dElectron_psai_zx > -pi/2-elecnnPzLimit) && (dElectron_psai_zx < -pi/2+elecnnPzLimit))||((dElectron_psai_zx > pi/2-elecnnPzLimit) && (dElectron_psai_zx < pi/2+elecnnPzLimit)))
	    && (dElectronPz_norm[0] > -elecnnPzLimit) && (dElectronPz_norm[0] < elecnnPzLimit)
	    )
	{   
	...
	double dIonPx1, dIonPy1,dIonPz1,dIonPx2,dIonPy2,dIonPz2, dElPx, dElPy, dElPz;
...
//	MessageBoxA(0,buffer,"TrueCount",MB_OK);
	...
	fprintf(fp1, "Tion1(z0) = %12.10f ns\n", tof(0, 0.0000354, dMass[0], dCharge[0]));
	fprintf(fp1, "Tion2(z0) = %12.10f ns\n", tof(0, 0.0000354, dMass[1], dCharge[1]));
	fprintf(fp1, "Telec = %12.10f ns\n", tctemp[2]*1.0E9);
	fprintf(fp1, "dTcyc = %12.10f ns \n", dTcyc*1.0E9);
	fprintf(fp1, "atan2(1.0,0.0) = %f\n", atan2(1.0,0.0));
	fprintf(fp1, "atan2(0.0,1.0) = %f\n", atan2(0.0,1.0));
	fprintf(fp1, "lEventCounter = %u\n", lEventCounter);
  fclose(fp1);
  double tempsqmd[2];
  double pz;
  int i; 
  fp1 = fopen("tcion.dat","wt");
  	for(i = 0; i < 100; i++)
  	{
  	   pz = ( i - 50.0 )/1.3E23;
  	   tempsqmd[0] = tof(pz, 0, dMass[0], dCharge[0]);
         tempsqmd[1] = tof(-pz, 0, dMass[1], dCharge[1]);
  	   fprintf(fp1, "%12.10f \t  %12.10f \n", tempsqmd[0]/Nano, tempsqmd[1]/Nano);
  	}
  	fprintf(fp1, "\n");
    fclose(fp1);
}
