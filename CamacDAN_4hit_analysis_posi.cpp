...
#include <math.h>
//#include <cstdlib>
FILE *fp1, *fp_cor, *fp_sel, *fp_mom;

////////////////  Please   ATTENTION !!!!!!!!   /////////////////////////////
////    This progrm is restricted version for 4hit.  ////////////////////////
////    AnaIon value must be fixed "4" in CCF now.   ////////////////////////
/////////////////////////////////////////////////////////////////////////////
#define	nTDC	3		// Number of TDC used.  Don't forget !! Input real number 
...
double AtomTOFWidth, AtomXwidth, AtomYwidth;

double xSq0_HotspotMin, xSq0_HotspotMax, ySq0_HotspotMin, ySq0_HotspotMax;	// Max and Min of 1st ion Hotspot (mm)
double xSq1_HotspotMin, xSq1_HotspotMax, ySq1_HotspotMin, ySq1_HotspotMax;	// Max and Min of 2nd ion Hotspot (mm)
double tIon0_RandCoinMin, tIon0_RandCoinMax, tIon1_RandCoinMin, tIon1_RandCoinMax;	// Max and Min of random coincidence line (ns)

double ion3TOFMin, ion3TOFMax;	//for Gate by 3rd ion TOF

double Del_PIPICOline_width12; //11/20に榊原がPIPICOにでる明らかにおかしい部分を消すために作った
double Del_PIPICOline_posi12; //11/27に榊原がPIPICOにでる明らかにおかしい部分を消すために作った
double Del_PIPICOline_width23; //11/20に榊原がPIPICOにでる明らかにおかしい部分を消すために作った
double Del_PIPICOline_posi23; //11/27に榊原がPIPICOにでる明らかにおかしい部分を消すために作った
/////////////////////////////////////////////////////////////////////////////
...
{
  double dVelec, dVion1st, dVion2nd, dViondet;
  ...
  dVelec      = pParameters->GetAt(213-1);  //V
  dVion1st    = pParameters->GetAt(214-1);  //V
  dVion2nd    = pParameters->GetAt(215-1);  //V
  dViondet    = pParameters->GetAt(216-1);  //V
	dElectric_Field		= (dVelec - dVion1st)/(dL + dD1);				//V/m
	dElectric_Field2  = (dVion1st - dVion2nd)/dL2;				//V/m
	dElectric_Field3  = (dVion2nd - dViondet)/dL3;
	dMagnetic_Field		= 1.0E-4 * pParameters->GetAt(217-1);		// Tesra
	//dBunch				= 1.0E-9 * pParameters->GetAt(215-1); 				// sec

	dAngle_Width		= pParameters->GetAt(218-1);				// width of plane angle (degree)
	...
	//To remove atomic ion line
	AtomTOFWidth		= 1.0E-9 * pParameters->GetAt(261-1);
	AtomXwidth			= 1.0E-3 * pParameters->GetAt(262-1);
	AtomYwidth			= 1.0E-3 * pParameters->GetAt(263-1);

	xSq0_HotspotMin     = pParameters->GetAt(249-1);	// mm //10/05
	xSq0_HotspotMax     = pParameters->GetAt(250-1);	// mm
	ySq0_HotspotMin     = pParameters->GetAt(251-1);	// mm
	ySq0_HotspotMax     = pParameters->GetAt(252-1);	// mm
	xSq1_HotspotMin     = pParameters->GetAt(253-1);	// mm
	xSq1_HotspotMax     = pParameters->GetAt(254-1);	// mm
	ySq1_HotspotMin     = pParameters->GetAt(255-1);	// mm
	ySq1_HotspotMax     = pParameters->GetAt(256-1);	// mm
	tIon0_RandCoinMin   = pParameters->GetAt(257-1);	// ns
	tIon0_RandCoinMax   = pParameters->GetAt(258-1);	// ns
	tIon1_RandCoinMin   = pParameters->GetAt(259-1);	// ns
	tIon1_RandCoinMax   = pParameters->GetAt(260-1);	// ns

	
	
	Del_PIPICOline_width12      = 1.0E-9 * pParameters->GetAt(267-1);	// 11/19 榊原がPIPICOに出てくる邪魔な部分を消去するために追加した。消去するラインの幅をこれで指定
	Del_PIPICOline_posi12      = 1.0E-9 * pParameters->GetAt(268-1);	// 11/27 榊原がPIPICOに出てくる邪魔な部分を消去するために追加した。消去するラインの位置をこれで指定
	Del_PIPICOline_width23      = 1.0E-9 * pParameters->GetAt(269-1);	// 11/19 榊原がPIPICOに出てくる邪魔な部分を消去するために追加した。消去するラインの幅をこれで指定
	Del_PIPICOline_posi23     = 1.0E-9 * pParameters->GetAt(270-1);	// 11/27 榊原がPIPICOに出てくる邪魔な部分を消去するために追加した。消去するラインの位置をこれで指定
...
//		fprintf(fp1, "%lf \n", dPosi_cor[i]);
//	}
    fp_sel = fopen("SelLucrand_ion3.dat","ab");
	
	fp_mom = fopen("Momenta_ion3_el1.dat","ab");
...
CDAN_API void AnalysisProcessEvent(CDoubleArray *pEventData,CDoubleArray *pParameters, CDoubleArray *pWeighParameter)
{
	...
	double dx0, dy0;
	double dtIon[nHit], dtIonN[2][nHit];
	...
	double dIonPzRaw[2][nHit];	// add 2013/09/19 find_pz_zを通す前のdIonPz 山田
...
//		dCOM_z[jjj] = tc0/2.0/dMass[0]*(dCharge[0]*dElectric_Field*tc0+an*sqrt(8.0)*sqrtA)-dL;
       ...
       //find_pz_z(dMass, dCharge, dtIonN[jjj], dCOM_z[jjj], dIonPz[jjj], iAnaIon);
		...
		for(int m=0; m<nHit; m++){
			dIonPzRaw[jjj][m] = dIonPz[jjj][m];
		}
		//find_pz_z(dMass, dCharge, dtIonN[jjj], dCOM_z[jjj], dIonPz[jjj], iAnaIon); //Pzの補正　実際のPzを使用するためコメントアウト13/09/19
...
// selection of electron TOF, the first electron
	for(iHit=0; iHit < electronhits; iHit++)
	{
...
//	for(iCount=0; iCount<iHitsTDC1; iCount++)
	...
	for(iCount=0; iCount < electronhits; iCount++)
	{	
...
//			theta = double(it)*dAngle_Width/2;
  ...
  else if(iPolarization==1)
  {
	...
	if((dIon_Total_Energy > ionEngMin) && (dIon_Total_Energy < ionEngMax) && 
		...
		&& (dtIon[2] > dMin_TOF[2]) && (dtIon[2] < dMax_TOF[2])//10/05
	    )
	{   
	...
	double dIonPx1, dIonPy1,dIonPz1,dIonPx2,dIonPy2,dIonPz2,dIonPx3,dIonPy3,dIonPz3, dElPx, dElPy, dElPz;
	dIonPx1 = dIonPx[jn][0]/dMomentum_au;
	dIonPy1 = dIonPy[jn][0]/dMomentum_au;
	//dIonPz1 = dIonPz[jn][0]/dMomentum_au;
	dIonPz1 = dIonPzRaw[jn][0]/dMomentum_au;
	dIonPx2 = dIonPx[jn][1]/dMomentum_au;
	dIonPy2 = dIonPy[jn][1]/dMomentum_au;
	//dIonPz2 = dIonPz[jn][1]/dMomentum_au;
	dIonPz2 = dIonPzRaw[jn][1]/dMomentum_au;
	//dIonPx3 = dIonPx[jn][2]/dMomentum_au;
	//dIonPy3 = dIonPy[jn][2]/dMomentum_au;
	//dIonPz2 = dIonPz[jn][1]/dMomentum_au;
	//dIonPz3 = dIonPzRaw[jn][2]/dMomentum_au;
	dElPx = dElectronPx[0]/dMomentum_au;
	dElPy = dElectronPy[0]/dMomentum_au;
	dElPz = dElectronPz[0]/dMomentum_au;

	if(iMaster_Flag>=0)
	{
		fwrite(&dIonPx1,sizeof(double),1,fp_mom);
		fwrite(&dIonPy1,sizeof(double),1,fp_mom);
		fwrite(&dIonPz1,sizeof(double),1,fp_mom);
		//fwrite(&dIonPx2,sizeof(double),1,fp_mom);
		//fwrite(&dIonPy2,sizeof(double),1,fp_mom);
		//fwrite(&dIonPz2,sizeof(double),1,fp_mom);
		//fwrite(&dIonPx3,sizeof(double),1,fp_mom);
		//fwrite(&dIonPy3,sizeof(double),1,fp_mom);
		//fwrite(&dIonPz3,sizeof(double),1,fp_mom);
		fwrite(&dElPx,sizeof(double),1,fp_mom);
		fwrite(&dElPy,sizeof(double),1,fp_mom);
		fwrite(&dElPz,sizeof(double),1,fp_mom);
	}
				////////////////////////
