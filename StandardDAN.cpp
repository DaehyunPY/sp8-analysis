...
double ion4TOFMin, ion4TOFMax;  //for Gate by 4th ion TOF
...
double Del_PIPICOline_width34; //11/20に榊原がPIPICOにでる明らかにおかしい部分を消すために作った
double Del_PIPICOline_posi34; //11/27に榊原がPIPICOにでる明らかにおかしい部分を消すために作った
/////////////////////////////////////////////////////////////////////////////
...
CDAN_API BOOL AnalysisInitialize(CDoubleArray *pEventData,CDoubleArray *pParameters, CDoubleArray *pWeighParameter)
{
  double dVelec, dVion1st, dVion2nd, dViondet;
    ...
    dMin_TOF[3]         = 1.0E-9 * pParameters->GetAt(265-1);
    dMax_TOF[3]         = 1.0E-9 * pParameters->GetAt(266-1);
  ...
  dViondet    = pParameters->GetAt(216-1);  //V
    ...
    xSq0_HotspotMin     = pParameters->GetAt(249-1);    // mm
    ...
    Del_PIPICOline_posi23     = 1.0E-9 * pParameters->GetAt(271-1); // 11/27 榊原がPIPICOに出てくる邪魔な部分を消去するために追加した。消去するラインの位置をこれで指定
    Del_PIPICOline_width34      = 1.0E-9 * pParameters->GetAt(272-1);   // 11/19 榊原がPIPICOに出てくる邪魔な部分を消去するために追加した。消去するラインの幅をこれで指定
    Del_PIPICOline_posi34     = 1.0E-9 * pParameters->GetAt(273-1); // 11/27 榊原がPIPICOに出てくる邪魔な部分を消去するために追加した。消去するラインの位置をこれで指定
...
//  }
    ...
    fp_sel = fopen("SelLucrand_ion4.dat","ab");
    return TRUE;
}
...
//          theta = double(it)*dAngle_Width/2;
  ...
  else if(iPolarization==1)
  {
    ...
    if((dIon_Total_Energy > ionEngMin) && (dIon_Total_Energy < ionEngMax) &&
        ...
        && (dtIon[2] > dMin_TOF[2]) && (dtIon[2] < dMax_TOF[2])
        && (dtIon[3] > dMin_TOF[3]) && (dtIon[3] < dMax_TOF[3])
        )
    {
