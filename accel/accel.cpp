...
sp8::AccMap::AccMap(sp8::Acc acc, double m, double q, double pfr, double pto, int bin)
    : acc(acc), m(m), q(q), pfr(pfr), pto(pto), bin(bin) {
...
double sp8::AccMap::getTAtP(double p) const {
  return get<1>(acc(m, q, p));
}
double sp8::AccMap::getPAtT(double t) const {
  if (!((tfr<t)&&(t<tto))) throw range_error("Arg t is over the range!");
...
std::tuple<double, double> sp8::AccMap::getPLimit() const {
  return {pfr, pto};
}
std::vector<double> sp8::AccMap::getPArr() const {
  return parr;
}
std::tuple<double, double> sp8::AccMap::getTLimit() const {
  return {tfr, tto};
}
